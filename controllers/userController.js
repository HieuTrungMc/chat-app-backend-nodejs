...
Controller.updateAccount = async (req, res) => {
    ...
    try {
        ...
        res.status(200).send(JSON.stringify({ok: 1, ...newInfo}))
    } catch (error) {
        console.log(error)
        res.status(400).send(JSON.stringify({ok: 0, message: "Error updating account"}))
    }
}
Controller.uploadImage = async (req, res) => {
    ...
    try {
        ...
        res.status(200).send(JSON.stringify({ok: 1, imageUrl}))
    } catch (error) {
        console.log(error)
        res.status(400).send(JSON.stringify({ok: 0, message: "Error uploading image"}))
    }
}
    ...
    try {
        ...
        if (account.length > 0) {
            if (account[0].password === password) {
                res.status(200).send(JSON.stringify(account[0]))
            } else {
                res.status(400).send(JSON.stringify({ok: 0, message: "Wrong password"}))
            }
        } else {
            res.status(400).send(JSON.stringify({ok: 0, message: "Phone number not found"}))
        }
    } catch (error) {
...
Controller.registerAccount = async (req, res) => {
    ...
    try {
        ...
        const account = await userModel.getAccountByPhone(phone)
        if (account.length > 0) {
            return res.status(400).send(JSON.stringify({ok: 0, message: "Phone number already exists"}))
        } else {
            ...
            res.status(200).send(JSON.stringify(newAccount))
        }
    } catch (error) {
    ...
    try {
        ...
        if (account.length > 0) {
            if (account[0].password === oldpassword) {
                const newAccount = await userModel.changePassword(account[0].id, newpassword);
                res.status(200).send(JSON.stringify(newAccount))
            } else {
                res.status(400).send(JSON.stringify({ok: 0, message: "Current password is wrong"}))
            }
        } else {
            res.status(400).send(JSON.stringify({ok: 0, message: "Phone number not found"}))
        }
